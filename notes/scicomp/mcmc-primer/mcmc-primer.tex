...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item On the other hand, when $\ve{x}$ has large tails, then the MALA chain is not exponentially ergodic.
    
    \item \begin{theorem}
      If $\pi_u$ is bounded and
      \begin{align*}
        \liminf_{\|\ve{x}\| \rightarrow \infty} \frac{\| \nabla \log \pi_u(\ve{x}) \|}{\| \ve{x}\|} > \frac{4}{\sigma^2},
      \end{align*}
      then the MALA chain is not exponentially ergodic.
    \end{theorem}

    \item \begin{theorem}
      If $\| \nabla \log \pi_u(\ve{x})\| \rightarrow 0$ as $\| \ve{x} \| \rightarrow \infty$, then the MALA chain is not geometricaly ergodic.
    \end{theorem}
  \end{itemize}

  \item For MALTA, the authors mentione that its exponential ergodicity is much more robust than that of MALA, but they did not give much details in the paper.
\end{itemize}
