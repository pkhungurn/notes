...
This article is written as I read the ``Deformation Transfer to Multi-Component Objects'' paper by Zhou \etal~\cite{Zhou:2009}.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Botsch \etal shows that the transfer in the classic paper can be achieved without having to add an extra vertex to each of the triangles, making the linear system smaller~\cite{Botsch:2006:DTDPSE}.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item The correspondence is determined by minimizing the following \emph{corresponding energy} with respect to the constraints above:
    \begin{align*}
    ...
    \item For each of the triangle $t^a_{i_k}$ above, we construct a tetrahedron using one of the three vertices of $t_{j_k}^b$ as the fourth vertex. We also form a tetra hedron for each of the triangles $t^b_{j_k}$. 
    \begin{itemize}
...
\begin{itemize}
    ...
    \item The loss terms $E_4$ in the last section is not needed if there are no orphan components.
    \begin{itemize}
    ...
    \item A later paper points out the algorithm makes an assumption that the special relationships between the parts are the same as the rest pose \cite{Ho:2010:SRPMA}.
    \begin{itemize}
        \item This is encoded by the $E_3$ term.

        \item This makes it unsuitable for movements where the spatial relationships between the parts would change like in animation whether body parts move relative to one another.        
    \end{itemize}
\end{itemize}
